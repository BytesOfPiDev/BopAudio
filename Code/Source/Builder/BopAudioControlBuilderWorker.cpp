...
namespace BopAudio
{
    ...
    AZ_TYPE_INFO_WITH_NAME_IMPL(BopAudioControlBuilderWorker, "BopAudioBuilserWorker", "{06B4A9AB-D205-4DAF-B36D-EEED181509B2}");
    ...
    namespace Internal
    {
        ...
        constexpr auto MalformedNodeMissingAttributeMessage{
            "%s node is malformed: does not have an attribute %s defined. This is likely the "
        ...
        [[maybe_unused]] constexpr auto MalformedNodeMissingChildNodeMessage{
            "%s node does not contain a child %s node. This is likely the result of manual editing. Please resave the Audio Control file.\n"
        };
        ...
        namespace Legacy
        {
            ...
            auto BuildConfigGroupMap(const AZ::rapidxml::xml_node<char>* preloadRequestNode, AtlConfigGroupMap& configGroupMap)
                -> AZ::Outcome<void, AZStd::string>
            {
                ...
                while (configGroupNode)
                {
                    ...
                    if (auto const configGroupNameAttr = configGroupNode->first_attribute(Audio::ATLXmlTags::ATLNameAttribute))
                    {
                ...
                AZStd::vector<AZStd::string>& banksReferenced) -> AZ::Outcome<void, AZStd::string>
            {
                ...
                while (preloadRequestNode)
                {
                    ...
                    auto const platformsNode = preloadRequestNode->first_node(Audio::ATLXmlTags::ATLPlatformsTag);
                    if (!platformsNode)
                    {
                        return AZ::Failure(AZStd::string::format(
                    ...
                    {
                        return AZ::Failure(AZStd::string::format(
                            MalformedNodeMissingChildNodeMessage, Audio::ATLXmlTags::ATLPlatformsTag, Audio::ATLXmlTags::PlatformNodeTag));
                    }
                    ...
                    while (platformNode)
                    {
                        auto const atlNameAttr = platformNode->first_attribute(Audio::ATLXmlTags::ATLNameAttribute);
                        if (!atlNameAttr)
                        {
                            return AZ::Failure(AZStd::string::format(
                                MalformedNodeMissingAttributeMessage,
                        ...
                        else if (atlPlatformIdentifier == atlNameAttr->value())
                        {
                            ...
                            //  the list
                            auto const configGroupNameAttr = platformNode->first_attribute(Audio::ATLXmlTags::ATLConfigGroupAttribute);
                    ...
                    if (!configGroupNode)
                    {
                        ...
                        AZ_TracePrintf(
                            ...
                            "%s node for config group %s is not defined, so no banks are referenced.",
                            Audio::ATLXmlTags::ATLConfigGroupTag,
        ...
        auto BuildAtlEventList(const AZ::rapidxml::xml_node<char>* triggersNode, AZStd::vector<AZStd::string>& eventNames)
            -> AZ::Outcome<void, AZStd::string>
        {
            ...
            while (triggerNode)
            {
                ...
                if (auto const eventNode = triggerNode->first_node(XmlTags::TriggerTag))
                {
                    ...
                    {
                        return AZ::Failure(
                            AZStd::string::format(MalformedNodeMissingAttributeMessage, XmlTags::TriggerTag, XmlTags::NameAttribute));
                    }
                }
        ...
        auto GetBanksFromAtlPreloads(const AZ::rapidxml::xml_node<char>* preloadsNode, AZStd::vector<AZStd::string>& banksReferenced)
            -> AZ::Outcome<void, AZStd::string>
        {
            AZ_Assert(preloadsNode != nullptr, NodeDoesNotExistMessage, Audio::ATLXmlTags::PreloadsNodeTag, "preload request");
            ...
            -> AZ::Outcome<void, AZStd::string>
        {
            ...
            {
                return AZ::Failure(
                    AZStd::string("The root of the metadata file is not an object. Please regenerate the metadata for this soundbank."));
            }
        ...
        auto GetEventsFromBank(AZStd::string const& bankMetadataPath, AZStd::set<AZStd::string>& eventNames)
            -> AZ::Outcome<void, AZStd::string>
        {
            if (!AZ::IO::SystemFile::Exists(bankMetadataPath.c_str()))
            {
                return AZ::Failure(AZStd::string::format(
                    "Failed to find the soundbank metadata file %s. Full dependency information cannot be determined without the metadata "
            ...
            {
                return AZ::Failure(AZStd::string::format(
                    "Failed to read the soundbank metadata file at path %s. Please make sure the file is not open or being edited by "
                    "another "
                    "program.",
                    bankMetadataPath.c_str()));
            }
    ...
    void BopAudioControlBuilderWorker::ProcessJob(
        [[maybe_unused]] AssetBuilderSDK::ProcessJobRequest const& request, AssetBuilderSDK::ProcessJobResponse& response)
    {
        ...
        AssetBuilderSDK::ProductPathDependencySet& pathDependencies)
    {
        ...
        if (!gatherBankReferencesResult)
        {
            AZ_Warning(
                "Audio Control Builder",
                false,
                "Failed to gather product dependencies for Audio Control file %s.  %s\n",
                sourceFile.c_str(),
        ...
        if (!gatherEventReferencesResult.IsSuccess())
        {
            AZ_Warning(
                "Audio Control Builder",
                false,
                "Failed to gather list of events referenced by Audio Control file %s. %s",
                sourceFile.c_str(),
        ...
        for (AZStd::string const& relativeBankPath : banksReferenced)
        {
            ...
            if (!getReferencedEventsResult.IsSuccess())
            {
                ...
                AZ_Warning(
                    "Audio Control Builder",
                    false,
                    "Failed to gather list of events referenced by soundbank %s. %s",
